...
export const GetList = (props) => {
  ...
  useEffect(() => {
    axios
      .get("https://localhost:8000/welapi/items/", {})
  ...
  const plusOne = (selectedId) => {
    ...
    // console.log(putData)
    axios
      .put(`https://localhost:8000/welapi/items/${plusItem[0].id}/`, putData, {
        headers: {
      ...
      .then((res) => {
        setItemList(
          itemList.map((item) => (item.id === selectedId ? res.data : item))
        );
      });
  };
  const minusOne = (selectedId) => {
    ...
    // console.log(putData);
    axios
      .put(`https://localhost:8000/welapi/items/${minusItem[0].id}/`, putData, {
        headers: {
          "Content-Type": "application/json",
        },
      })
      .then((res) => {
        setItemList(
          itemList.map((item) =>
            item.id === minusItem[0].id ? res.data : item
          )
        );
      });
  };
  ...
  const DeleteItems = () => {
    ...
    const promise = new Promise((resolve, reject) => {
      selectedItem.map((itemID) => {
        axios.delete(`https://localhost:8000/welapi/items/${itemID}/`, {
          headers: {
